...
//   So we do the email-sending here on Vercel, and OTP verification stays on Render.
//
// FLOW:
//   1. Frontend calls POST /api/send-otp with { userId, email, name }
//   2. This function generates a 6-digit OTP
//   3. Hashes and saves OTP to MongoDB (same Atlas cluster the Render backend uses)
//   4. Sends the email via Gmail SMTP
//   5. Render backend's /api/auth/verify-otp reads + verifies OTP from MongoDB (unchanged)
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px; border: 1px solid #1e293b; border-radius: 12px; background-color: #0f172a; color: #f8fafc;">
      ...
      <div style="padding: 0 10px;">
        ...
        <p style="color: #94a3b8; font-size: 14px; line-height: 1.6;">
          Thank you for signing up for Delhi MetroPulse. To activate your account, please verify your email address using the code below.
        </p>
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Extract DB name from the connection string (e.g. .../metro_tracker?...)
