...
export default function SearchStationPage() {
  ...
  return (
    <div className="bg-gradient-to-b from-[#F6FBF7] via-[#fcf8ff] to-[#f4f7fc] min-h-screen relative flex flex-col pt-safe">
      ...
      <main className="flex-1 overflow-y-auto px-6 pb-6">
        ...
        {othersToDisplay.length > 0 && (
          <div className="mt-4 mb-2">
            <span className="text-[11px] font-bold text-[#4CAF50] uppercase tracking-wider ml-1">All Stations</span>
          </div>
        )}
        ...
        {othersToDisplay.map((stationName) => (
          <div 
            ...
            className="flex items-center py-4 border-b border-gray-50 cursor-pointer active:bg-white/40 transition-colors rounded-xl px-2"
          >
            <span className="material-symbols-outlined text-[#4CAF50] mr-4 text-[22px]" style={{ fontVariationSettings: "'FILL' 1" }}>
              location_on
            </span>
            <span className="text-gray-800 text-[15px] font-medium">{stationName}</span>
          </div>
