...
function StationInput({ label, value, onChange, onSelect, stations, icon: Icon }) {
  ...
  const filtered = stations.filter((s) => s.toLowerCase().includes(value.toLowerCase()) && value.length > 0);
  ...
  return (
    <div className="relative">
      ...
      {open && filtered.length > 0 && (
        <div className="absolute z-50 w-full mt-1 glass-card overflow-hidden max-h-48 overflow-y-auto">
          {filtered.slice(0, 8).map((name) => (
            <button
              ...
              onMouseDown={() => { onSelect(name); setOpen(false); }}
            >
              <Train className="w-3 h-3 text-metro-accent flex-shrink-0" />
              {name}
            </button>
          ))}
        </div>
