 ...
 * provides BFS-based shortest path and Dijkstra-based line/distance/congestion optimizers
 */
class MetroGraph {
  constructor() {
  ...
  /**
   * Internal helper to resolve fuzzy, case-insensitive, or typo-filled station inputs
   */
  _getExactStationName(name) {
    ...
    // 2. Try substring matching (where key contains query, e.g. "Kashmere" matches "Kashmere Gate")
    ...
    if (substringCandidates.length > 0) {
      // Sort candidates by length (shortest first) so that the most specific/closest match is preferred
      substringCandidates.sort((a, b) => a.length - b.length);
      return substringCandidates[0];
    }

    // 3. Try word-by-word fuzzy prefix matching (e.g. "Kashmeri" matches "Kashmere Gate")
    const wordCandidates = [];
    const queryWords = lowerName.split(/[\s()\-]+/);
    for (const key of this.graph.keys()) {
      const lowerKey = key.toLowerCase();
      const stationWords = lowerKey.split(/[\s()\-]+/);
      
      for (const qw of queryWords) {
        if (qw.length < 3) continue;
        for (const sw of stationWords) {
          if (sw.length < 3) continue;
          const dist = this._getLevenshteinDistance(qw, sw);
          if (dist <= 2) {
            wordCandidates.push({ key, dist });
          }
        }
      }
    }
    if (wordCandidates.length > 0) {
      // Sort by edit distance first, then by key length
      wordCandidates.sort((a, b) => a.dist - b.dist || a.key.length - b.key.length);
      return wordCandidates[0].key;
    }

    // 4. Try global Levenshtein distance <= 3
    let bestMatch = null;
  ...
  /**
   * Private internal Haversine distance calculator to avoid circular import issues
   */
   ...
   * Returns path, interchanges, and estimated time (fewest stations)
   */
  findShortestPath(sourceInput, destInput) {
    ...
    if (!sourceName) {
      return { error: `Source station "${sourceInput}" not found` };
    }
    if (!destName) {
      return { error: `Destination station "${destInput}" not found` };
    }
  ...
  /**
   * Dijkstra-based Minimum Interchanges Algorithm
   * Minimizes train transfers (line switches)
   */
  findMinInterchangesPath(sourceInput, destInput) {
    ...
    while (pq.length > 0) {
      ...
      if (cost > (visited.get(`${station}:${line}`) ?? Infinity)) {
        continue;
      }
      ...
      for (const neighborName of neighbors) {
        ...
        if (neighbor.line !== line) {
          weight = 1000; // Line switch occurs
    ...
    if (bestPath) {
      return this._buildResult(bestPath);
    }
  ...
  /**
   * Dijkstra-based Shortest Physical Distance Algorithm
   * Minimizes physical travel distance in kilometers (using Haversine)
   */
  findShortestDistancePath(sourceInput, destInput) {
    ...
    while (pq.length > 0) {
      ...
      if (cost > (visited.get(station) ?? Infinity)) {
        continue;
      }
      ...
      for (const neighborName of neighbors) {
        ...
        // Weight is physical distance in meters between station and neighbor
        const distance = (currStation.lat && currStation.lng && neighbor.lat && neighbor.lng)
          ...
          : 2000; // default to 2km if coordinates missing
  ...
  /**
   * Dijkstra-based Less Congested Route Algorithm
   * Prefers less crowded paths, dynamically bypassing crowded interchanges
   */
  findLessCongestedPath(sourceInput, destInput) {
    ...
    while (pq.length > 0) {
      ...
      for (const neighborName of neighbors) {
        ...
        // Congestion score of the target station (multiplier from 1.0 to 5.0)
        const congestionScore = congestionService.getCongestionScore(neighborName);
        
        // Edge cost is 1 stop multiplied by target congestion score
   ...
   * Symmetric and accurate interchange and physical distance computation
   */
  _buildResult(path) {
    const interchanges = [];
    ...
    return {
      ...
      stationDetails: path.map((name) => {
        ...
        const score = congestionService.getCongestionScore(name);
        return {
          ...station,
          congestion: {
            score,
