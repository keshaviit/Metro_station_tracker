...
function buildEmailHtml(name, otp) {
  return `
    <div style="font-family:'Segoe UI',sans-serif;max-width:500px;margin:0 auto;padding:20px;border:1px solid #1e293b;border-radius:12px;background:#0f172a;color:#f8fafc;">
      <div style="text-align:center;border-bottom:1px solid #334155;padding-bottom:20px;margin-bottom:25px;">
        <span style="font-size:32px;">🚇</span>
        <h1 style="margin:10px 0 0;font-size:24px;color:#6366f1;font-weight:800;">Delhi MetroPulse</h1>
        <p style="margin:5px 0 0;font-size:11px;text-transform:uppercase;letter-spacing:1.5px;color:#94a3b8;">Email Verification</p>
      </div>
      <div style="padding:0 10px;">
        <h2 style="font-size:18px;margin-top:0;color:#fff;">Welcome, ${name}!</h2>
        <p style="color:#94a3b8;font-size:14px;line-height:1.6;">Use the code below to verify your email and activate your Delhi MetroPulse account.</p>
        <div style="background:#1e293b;border:1px solid #334155;border-radius:8px;padding:20px;text-align:center;margin:25px 0;">
          <p style="margin:0 0 10px;font-size:12px;text-transform:uppercase;letter-spacing:2px;color:#6366f1;font-weight:700;">Your One-Time Passcode</p>
          <span style="font-size:38px;font-weight:900;letter-spacing:8px;color:#fff;font-family:monospace;">${otp}</span>
          <p style="margin:10px 0 0;font-size:11px;color:#f43f5e;">Valid for 10 minutes</p>
        </div>
...
module.exports = async function handler(req, res) {
  ...
  if (!process.env.MONGODB_URI) {
    console.error('[send-otp] Missing MONGODB_URI env var');
    return res.status(500).json({ success: false, message: 'Database not configured on Vercel' });
  }
  if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) {
    console.error('[send-otp] Missing GMAIL env vars');
  ...
  try {
    ...
    console.error('[send-otp] Invalid userId format:', userId);
  ...
  try {
    ...
    console.log('[send-otp] Updating OTP in DB for userId:', userId);
    ...
    if (updateResult.matchedCount === 0) {
      console.error('[send-otp] User not found in DB:', userId);
    ...
    console.error('[send-otp] Runtime error:', err.message, err.stack);
